// GameSeed.cs
// ------------------------------------------------------------
// Full updated seed based on your latest direction:
//
// ✅ NO Contracts / Eggs
// ✅ NO LinkedMonsterInstanceId / LinkedMercenaryInstanceId usage
// ✅ NO ItemType / MercenarySlot / MonsterSlot usage in ItemTemplate
// ✅ Uses ONE unified EquipSlot enum (1–99 Merc, 100–199 Beast)
// ✅ OwnerKind is derived from EquipSlot range
// ✅ Seeds:
//    - Identity (admin users)
//    - Locations (Orders JSON) + AI generation saved to DB
//    - Dungeons (Beasts JSON) + AI generation saved to DB
//    - ItemTemplates (items_full.json) -> ItemTemplate with EquipSlot + OwnerKind
//
// ⚠️ Assumptions (must exist in Domain):
// - ItemTemplate has: Code, NameEn, BaseQuality, BaseStats, OwnerKind, EquipSlot
// - ItemOwnerKind enum has: None, Mercenary, Beast
// - ItemEquipSlot enum exists (shown below) or in Domain.Enums
// - Location/Dungeon entities and your AI generator accept/modify them like before
// ------------------------------------------------------------
...
public sealed class GameSeed
{
    ...
    public async Task SeedAsync(bool makeAll = false,CancellationToken ct = default)
    {
        ...
        // 2) Locations + Dungeons from your JSONs (Orders/Beasts) using AI generator
        await SeedLocationsAndDungeonsFromLabyrinthJsonAsync(ct);

        // 3) ItemTemplates from items_full.json
...
{
    // Pokud nechceš makeAll a DB už má aspoň něco, můžeš vypadnout.
    var locs = _db.Locations.Count();
    var dungs = _db.Dungeons.Count();
    if (!makeAll && await _db.Locations.AnyAsync(ct) && await _db.Dungeons.AnyAsync(ct))
        return;
    ...
    // quick gate (volitelné)
    if (!makeAll)
    {
        var dbLocationCount = await _db.Locations.CountAsync(ct);
        var dbDungeonCount = await _db.Dungeons.CountAsync(ct);
        if (dbLocationCount >= ordersById.Count && dbDungeonCount >= beastsById.Count)
            return;
    }

    // --------------------------
    // Locations (Orders) first
    // --------------------------
    ...
    foreach (var (labId, orderRows) in ordersById.OrderBy(kv => kv.Key))
    {
        ...
        // ✅ KLÍČ: Naplň předdefinované Stage mercenaries (jen v paměti),
        // aby GenerateNextLocationAsync použil "STAGES / MERCENARIES (STRICT)" blok.
        //AttachPredefinedMercStages(loc, orderRows);
    ...
    foreach (var (labId, beastRows) in beastsById.OrderBy(kv => kv.Key))
    {
        ...
        // ✅ KLÍČ: Naplň předdefinované Stage beasts (jen v paměti),
        // aby GenerateNextDungeonAsync použil "STAGES / BEASTS (STRICT)" blok.
        AttachPredefinedBeastStages(dng, beastRows);
        var beastssCsv = string.Join(", ",
                beastRows
                    .Select(x => x.beast)
                    .Where(o => !string.IsNullOrWhiteSpace(o))
                    .Distinct(StringComparer.OrdinalIgnoreCase)
            );
        await _ai.GenerateNextDungeonAsync( null, dng, true, beastssCsv, ct);
        await _db.SaveChangesAsync(ct);
    ...
    {
        // kolik stage typů očekáváš (generator to vyžaduje)
        var stageCount = Enum.GetNames(typeof(ExpeditionStageType)).Length;

        // zkus z row vytáhnout (stageIndex, mercName)
        var pairs = rows
        ...
        // fallback: když stageIndex není, aspoň vezmi unikátní jména a přiřaď 1..N
        if (pairs.Count == 0)
        {
            var names = rows
            ...
            for (int i = 0; i < names.Count; i++)
            {
                ...
                {
                    // Id není potřeba (jen pro prompt), ale když chceš:
                    Id = Guid.NewGuid(),
                    LocationId = loc.Id,
                    StageNumber = i + 1,
        ...
        // skupiny podle stage, vezmi první jméno na stage
...
private static string ToCode(string prefix, string name)
{
    ...
    foreach (var ch in upper)
    {
        ...
        else sb.Append('_'); // mezery, diakritika, cokoliv → _
    }
    ...
    public sealed record ItemRow(
        ...
        [property: JsonPropertyName("itemType")] string? ItemType, // ignored for now
...
private async Task SeedItemTemplatesFromItemsFullJsonAsync(CancellationToken ct)
{
    ...
    var rows = file?.Items ?? new List<ItemRow>();
        ...
        .SelectMany(g => g
            .OrderBy(x => (x.Name ?? "").Trim()) // deterministicky
            .Take(5))
        .ToList();
        }
    foreach (var r in rows)
    {
        ...
        if (string.IsNullOrWhiteSpace(r.Group))
            continue; // nebo throw - podle toho jak přísně chceš hlídat data
        ...
        // idempotence: podle Code (nebo podle slotu, ale Code je ok)
        ...
        var tpl = new ItemTemplate
        {
            ...
            // 👇 model má MercenarySlot/MonsterSlot
            MercenarySlot = ownerKind == ItemOwnerKind.Mercenary ? equipSlot : null,
            MonsterSlot   = ownerKind == ItemOwnerKind.Beast     ? equipSlot : null
        };

        // 2) AI enrich (popis + bonusy + případně upraví NameEn)
        // theme hint můžeš dát z group (lepší prompt)
    ...
    private async Task SeedItemTemplatesViaAiAsync(CancellationToken ct)
{
    ...
    foreach (var slot in allSlots)
    {
        ...
        // najdi existující podle slotu (podle tvého modelu)
        ...
        // pokud existuje, nech (nebo update, jak chceš)
        if (existing != null)
            continue;

        // hint můžeš navázat na element lokace/dungeonu nebo theme
    ...
    private async Task SeedCountriesFromGeoNamesAsync(CancellationToken ct)
{
        ...
        LocalizedNameResult res)
    {
        ...
        foreach (var culture in LocalizationConfig.SupportedLocales)
        {
            ...
            // Pokud není EN description, necháme všechny localized description null
        ...
        string? description)
    {
        ...
        // Description updatni i na null (pokud chceš "nepřepisovat", změň to)
...
{
    // kdyz makeAll=false: jen maly vzorek, abys nespalil tokeny
    // kdyz makeAll=true: vse
    ...
    where T : class
{
    ...
    // 1) stáhni jen to, co potřebuješ (v SQL)
    ...
    foreach (var e in list)
    {
        ...
        // jestli "en" nechceš ukládat do LocalizedTexts, tak ho z SupportedLocales vyhoď
    ...
    // pokud je jich hodně, náhodně omez
    if (missing.Count > pocet)
    {
        // Fisher–Yates shuffle (bez LINQ overheadu)
